...
class Market extends AdminPages
{
	...
	public function index ()
	{
		...
		$data['data'] = $this->ModelsMarket->getBuy();
		foreach ($data['data'] as $key => $value):
			$data['data'][$key]->cat = $this->ModelsMarket->getCat($value->id);
		endforeach;
		$this->set($data);
	...
	public function add ()
	{
		...
		$menu[] = array(CATEGORIES => array('href'=>'/market/categories?management&pages','icon'=>'fa fa-cogs'));
		$menu[] = array(CONFIG     => array('href'=>'/market/parameter?management&pages','icon'=>'fa fa-cubes'));
		$data['cat'] = $this->ModelsMarket->getAllCat();
		$this->set($data);
	...
	{
		$return = $this->ModelsMarket->sendBuy ($_POST);
	...
	{
		$return = $this->ModelsMarket->delcat ($data);
		$this->error(get_class($this), $return['text'], $return['type']);
	...
	{
		$d['data']   = current($this->ModelsMarket->getCat($id));
		$d['groups'] = BelCMSConfig::getGroups();
		$this->set($d);
		$this->render('editcat');
	}
	...
	{
		$return = $this->ModelsMarket->sendeditcat ($_POST);
		$this->error(get_class($this), $return['text'], $return['type']);
		$this->redirect('/market?management&pages', 2);
	}
	#########################################
	...
	{
		$return = $this->ModelsMarket->sendaddCat ($_POST);
		$this->error(get_class($this), $return['text'], $return['type']);
		$this->redirect('/market?management&pages', 2);
	}
	#########################################
	...
	{
		$data['data'] = $this->ModelsMarket->getBuy($id);
		$data['cat']  = $this->ModelsMarket->getAllCat();
		$this->set($data);
		$this->render('editbuy');	
	}
	...
	{
		$return = $this->ModelsMarket->sendEditBuy ($_POST);
		$this->error(get_class($this), $return['text'], $return['type']);
		$this->redirect('/market?management&pages', 2);	
	}
	#########################################
